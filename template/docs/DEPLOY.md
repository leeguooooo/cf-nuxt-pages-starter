...
`wrangler:config:*` 会把对应账号的 `wrangler.account-*.toml` 复制为根目录的 `wrangler.toml`，因为 Pages 只能读取默认配置文件。默认部署目录为 `dist`（Nuxt 4 + Cloudflare Pages 的产物）。
...
- **部署目录错误**：模板使用 `dist` 作为输出路径；如果改成别的目录，记得同步 `wrangler.account-*.toml` 与部署脚本。
